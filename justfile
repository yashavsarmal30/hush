...
release tag="":
    node -e "const tag = process.argv[1] || ('v' + require('./package.json').version); console.log('\x1b[32m--> Creating release tag:\x1b[0m', tag); require('child_process').execSync('git tag ' + tag + ' && git push origin ' + tag, { stdio: 'inherit' }); console.log('\x1b[36m--> Release tag pushed! GitHub Actions CI/CD will now build and publish release artifacts.\x1b[0m');" "{{tag}}"
